...
class BFSSolution:
    def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        ...
        self.init_all_general_forms(wordList)
        ...
        while queue:
            ...
            for i in range(self.word_length):
                general_form = current[:i] + '*' + current[i+1:]
                for w in self.all_general_forms[general_form]:
                    if w == endWord:
                        return level + 1
                    if w not in visited:
                        visited[w] = True
    ...
    def init_all_general_forms(self, wordList):
        self.all_general_forms = collections.defaultdict(list)
        for w in wordList:
            for i in range(self.word_length):
                form = w[:i] + '*' + w[i+1:]
                self.all_general_forms[form].append(w)
...
class FinalSolution:
    ...
    def ladderLength(self, beginWord: str, endWord: str, wordList: List[str]) -> int:
        ...
        queue_beign = collections.deque([(beginWord, 1)])
        queue_end = collections.deque([(endWord, 1)])
        visited_begin = {beginWord: 1}
        visited_end = {endWord: 1}

        while queue_beign and queue_end:
            length = self.check_visited(queue_beign, visited_begin, visited_end)
            if length:
                return length
            length = self.check_visited(queue_end, visited_end, visited_begin)
            if length:
                return length
        return 0
    ...
    def check_visited(self, queue, visited, other_visited):
        current, level = queue.popleft()
        for i in range(self.word_length):
            form = current[:i] + '*' + current[i+1:]
            for w in self.all_general_forms[form]:
...
if __name__ == '__main__':
    ...
    ans = 11
    length = FinalSolution().ladderLength(beginWord, endWord, wordList)
    print(length)
